...
/// Unknown
fn use_tracing_futures(
    w: impl for<'writer> tracing_subscriber::fmt::MakeWriter<'writer> + 'static + Send + Sync,
) {
    use tracing::info;

    let _g = std::thread::spawn(move || {
        let (_r, _d) = crate::library::create_future_log(w, move || -> Result<(), String> {
            info!("log 1");
...
pub fn use_tracing_futures_stdout() {
    use_tracing_futures(crate::library::create_stdout());
}

pub fn use_tracing_futures_file() {
    let (w, _g) = crate::library::create_appender("log1", ".logs");
    use_tracing_futures(w);
}
